...
def main(event, context):
    ...
    for record in event['Records']:
        ...
        if len(path) == 4:
            ...
            updated_fields = json.loads(bucket.get_object(Key=record['s3']['object']['key'])['Body'].read().decode('utf-8'))
            query_list = []
            for updated_field in updated_fields:
                try:
                    query_list.extend(json.loads(bucket.get_object(Key='vector/{record_type}/{updated_field}.json'.format(record_type=record_type, updated_field=updated_field))['Body'].read().decode('utf-8')))
                except:
                    pass
            query_list = sorted(list(set(query_list)))
            for query_id in query_list:
                query_result = json.loads(lambda_client.invoke(FunctionName=query_id, InvocationType='RequestResponse', Payload=bytes(json.dumps(record_object), 'utf-8'))['Payload'].read().decode('utf-8'))
