...
def main(event, context):
    ...
    - returns {status: '202'} on success or {status: '500'} | {status: '400'} on failure
    '''
    status = 202
    try:
        ...
        for request_object in [r['cf']['request'] for r in event.get('Records', [])]:
            ...
            if status == 202:
                ...
                if status == 202:
                    ...
                    if status == 202:
                        try:
                            s3_client.put_object(Bucket=bucket_name, Key='{}/{}.json'.format(path, request_uuid), Body=bytes(json.dumps(request_entry), 'utf-8'), ContentType='application/json')
                        except: 
                            status = 500
