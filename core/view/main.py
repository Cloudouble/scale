        ...
        if view and type(view.get('processor')) is str:
            ...
            if view.get('assets'):
                for asset_path in view['assets'].values():
                    if lambda_client.invoke(FunctionName='{lambda_namespace}-core-mask'.format(lambda_namespace=env['lambda_namespace']), Payload=bytes(json.dumps({'connection_id': connection_id, 'entity_type': 'asset', 'path': asset_path, 'method': 'GET'}), 'utf-8'), ClientContext=client_context)['Payload'].read().decode('utf-8'):
                        bucket.copy({'Bucket': env['bucket'], 'Key': '{data_root}/asset/{asset_path}'.format(data_root=env['data_root'], asset_path=asset_path)}, 
                            '{data_root}/connection/{connection_id}/asset/{asset_path}'.format(connection_id=connection_id, asset_path=asset_path))
            if entity_type == 'record':
                masked_record_data = json.loads(bucket.Object('{data_root}/connection/{connection_id}/record/{class_name}/{entity_id}.json'.format(data_root=env['data_root'], connection_id=connection_id, class_name=class_name, entity_id=entity_id)).get()['Body'].read().decode('utf-8'))
                if field_name:
                    view_result_key = '{data_root}/connection/{connection_id}/subscription/{class_name}/{entity_id}/{field_name}.{suffix}'.format(data_root=env['data_root'], connection_id=connection_id, class_name=class_name, entity_id=entity_id, field_name=field_name, suffix=suffix)
                else:
                    view_result_key = '{data_root}/connection/{connection_id}/subscription/{class_name}/{entity_id}.{suffix}'.format(data_root=env['data_root'], connection_id=connection_id, class_name=class_name, entity_id=entity_id, suffix=suffix)
                ...
                query_base_key = '{data_root}/connection/{connection_id}/query/{class_name}/{entity_id}/'.format(data_root=env['data_root'], connection_id=connection_id, class_name=class_name, entity_id=entity_id)
                ...
                for record_id in query_record_ids:
                    query_record_data.append(json.loads(bucket.Object('{data_root}/connection/{connection_id}/record/{class_name}/{record_id}.json'.format(data_root=env['data_root'], connection_id=connection_id, class_name=class_name, record_id=record_id)).get()['Body'].read().decode('utf-8')))
                ...
                for page_name, page_results in {'{}-{}'.format(i*1000, i*1000+999): p for i, p in enumerate(list(chunks(query_record_data, 1000)))}.items():
                    ...
                    if field_name:
                        page_object_key = '{data_root}/connection/{connection_id}/feed/{class_name}/{entity_id}/{field_name}/{sort_field}/{sort_direction}/{page_name}.{suffix}'.format(data_root=env['data_root'], **switches)
                    else:
                        page_object_key = '{data_root}/connection/{connection_id}/feed/{class_name}/{entity_id}/-/{sort_field}/{sort_direction}/{page_name}.{suffix}'.format(data_root=env['data_root'], **switches)
