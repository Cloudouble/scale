    ...
    if all([event.get(k for k in ['connection_id', 'class_name', 'entity_type', 'entity_id']]) and type(event.get('view')) is dict:
        ...
        if view and type(view.get('processor')) is str:
            ...
            if entity_type == 'record':
                ...
                query_record_data.sort(key=sort_field, reverse=True if view.get('sort_direction', 'ascending') == 'descending' else False)
                ...
                for page_name, page_results in {'{}-{}'.format(i*1000, i*1000+999): p for i, p in enumerate(list(chunks(query_record_data, 1000)))}:
                    ...
                    }), 'utf-8'))['Payload'].read(), Key=page_object_key, ContentType=view.get('content_type', 'application/json'))
                counter = counter + 1
    return counter
