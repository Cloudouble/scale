...
for functionName in *; do
    ...
    if [ ${#existingFunctionArn} -ge 10 ]; then
        ...
        cp main.py ./temp
        cd temp
        if [ 'request' = $functionName -o 'socket' = $functionName -o 'channel' = $functionName -o 'channel-send' = $functionName -o 'console' = $functionName ]; then
...
echo "Ensuring CloudFront distribution is created and configured correctly..."
    ...
    PathPatternOrder=(
        ...
        "$envSystemRoot/channel/????????-????-????-????-????????????/????????-????-????-????-????????????::s"
        "$envSystemRoot/connection/????????-????-????-????-????????????/channel/????????-????-????-????-????????????/connect.json::w"
        "$envSystemRoot/connection/????????-????-????-????-????????????/channel/????????-????-????-????-????????????/????????-????-????-????-????????????::w"
