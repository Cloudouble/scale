...
def main(event, context):
    '''
    - triggered by new/updated/deleted _/{connection_type}/{connection_id}/query/{class_name}/{query_id}/{index_initial}.json
    ...
    if event.get('key'):
        ...
        if len(env['path']) == 6:
            connection_type, connection_id, entity_type, class_name, query_id, index = env['path']
