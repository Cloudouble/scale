...
window.setTimeout(() => {
    var installationTable = document.getElementById('tests').querySelector('table[name="installation"]'), 
        installationTableTbody = installationTable.querySelector('tbody')
    Object.entries(window.LiveElement.Scale.Console.Tests.testMap).forEach(entry => {
        ...
        testTd.innerHTML = `<label>${entry[1].resultLabel}<code class="result"></code></label><label>${entry[1].confirmationLabel}<code class="confirmation"></code></label>`
        ...
        if (window.localStorage.getItem(confirmationStorageKey)) {
            ...
            testTd.querySelector('label[name="confirmation"] code').innerHTML = window.localStorage.getItem(confirmationStorageKey)
        }
        installationTableTbody.appendChild(testTr)
        ...
        installationTableTbody.appendChild(snippetTr)
    })
}, 1)

window.LiveElement.Scale.Console.Tests.testMap = {
    'create-sudo-connection': {
        runner: function(connection_url, system_access_url, system_root, connection_id) {
